...
lastmod: 2026-05-04T12:20:00+08:00
draft: false
summary: "以 Hung-yi Lee 的 BP.pdf 为反向传播主线：先解释 SGD 为什么要整条梯度、forward 怎样保存局部斜率、backward 怎样递推每个 z 的 adjoint，再用统一的 L、θ、Δ_u 记号逐层推导 Transformer 的 Embedding、Self-Attention、MHA、LayerNorm、LoRA 与 GPT block。"
...
> 里的课件与白板材料，尤其按 `BP.pdf` 的 forward/backward 两遍算法把普通
> 神经网络反向传播讲清楚；再进入 Laurent Boué
...
| $z$ | BP 课件里的 activation function input，即非线性前的 pre-activation |
| $a=\sigma(z)$ | activation output，也是下一层线性加权的输入 |
...
`BP.pdf` 还特意把 dataset 维度摊开：如果每个样本的代价是 $C_n(\theta)$，
总损失就是

$$
L(\theta)=\sum_{n=1}^N C_n(\theta),
\qquad
\frac{\partial L}{\partial w}
=\sum_{n=1}^N\frac{\partial C_n}{\partial w}.
$$

所以后面所有“单样本、单神经元”的反向规则，放到 batch 训练时都只是沿样本维
再做一次求和。Transformer 里 $X^T\Delta$ 这类矩阵乘法，本质上就是把每个 token
或每个样本贡献的外积累加起来。
...
## Stage 1 · 按 BP.pdf 读反向传播：局部斜率乘全局误差信号

`BP.pdf` 的核心解释不是“自动求导系统很神奇”，而是把一个权重 $w$ 的梯度拆成
两项。假设某个神经元的 pre-activation 是
...
=\frac{\partial z}{\partial w_i}\frac{\partial L}{\partial z}.
$$

这就是整份 BP 课件的主线：forward pass 负责给出局部斜率
$\partial z/\partial w_i$，backward pass 负责给出这个节点从最终损失接收到的
全局误差信号 $\partial L/\partial z$。

{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F4.svg" label="F4" caption="BP.pdf 的核心等式：每个权重梯度都是 forward 局部斜率乘 backward 全局 adjoint。" >}}
...
$$
\frac{\partial z}{\partial w_i}=a_i,
\qquad
\frac{\partial z}{\partial b}=1.
$$

这解释了为什么 forward 要保存 activation。它不是为了“记流水账”，而是因为
每条入边权重的梯度都要用到这条边左侧的 activation。把这个规则扩展到 batch，
就是线性层公式 $\partial L/\partial W=X^T\Delta$。

再看 backward。BP 课件把 $z$ 后面的 activation 写成

$$
a=\sigma(z),
$$

并假设 $a$ 继续通过权重 $w_3,w_4$ 影响下游的 $z'$、$z''$。如果下游的
$\Delta_{z'}$、$\Delta_{z''}$ 已经知道，那么

{{< formula type="sm" label="BP.pdf hidden-neuron recursion" >}}
$$
\Delta_z
=\sigma'(z)\left(w_3\Delta_{z'}+w_4\Delta_{z''}\right).
$$
{{< /formula >}}
...
$$
\Delta_z
=\sigma'(z)\sum_j w_j\Delta_{z'_j}.
$$

这里有两个细节很重要：

1. $\sigma'(z)$ 里的 $z$ 是 forward 已经算过的值，所以反向只需要读取或重算它。
2. 下游有几条路，就把几条路的贡献加起来；这正是后面 residual、multi-head、
   $Q/K/V$ 三分支都会重复出现的规则。

最后，forward 与 backward 在每条权重边上相乘：

{{< formula type="sm" label="BP.pdf weight-gradient summary" >}}
$$
\frac{\partial L}{\partial w_i}=a_i\Delta_z.
$$
{{< /formula >}}

这条式子是后面所有矩阵公式的“标量原型”。线性层的 $X^T\Delta_A$、attention
投影的 $X^T\Delta_Q$、LoRA 的 $X^T\Delta_YU^T$，都可以读成很多个
$a_i\Delta_z$ 外积的批量累加。

## Stage 2 · 从 BP 神经元到计算图：每个节点只问两个问题
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F5.svg" label="F5" caption="计算图反向传播的核心规则：一个值流向多个下游节点时，所有路径的梯度贡献要相加。" >}}
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F6.svg" label="F6" caption="反向模式的每个节点只做局部 VJP；forward 保存的中间值让 backward 不必重新展开整张图。" >}}

## Stage 3 · 统一模板：每层 backward 都是整理 $\Delta:dA$
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F7.svg" label="F7" caption="反向传播的统一模板：先写微分，再把 $\Delta_A:dA$ 改写成各个变量的梯度。" >}}
...
## Stage 4 · Embedding：前向是查表，反向是 scatter-add
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F8.svg" label="F8" caption="Embedding 的 OHE 写法只是推导工具；实现里对应被选中行的 scatter-add。" >}}
...
## Stage 5 · Self-Attention 前向：先看对象，再看依赖
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F9.svg" label="F9" caption="单头 attention 的前向图：Q/K 决定注意力分布，V 被分布加权混合。" >}}
...
## Stage 6 · Self-Attention 反向：$\Delta_Y$ 分三条路回到 $X$
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F10.svg" label="F10" caption="Attention 反向：输出梯度先分到 V 与 P，P 再经 softmax 回到 Q/K score。" >}}
...
## Stage 7 · Key bias 为什么没有意义：softmax 的平移不变性
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F11.svg" label="F11" caption="Key bias 失效的原因：它只给每一行 score 加常数，而 row-softmax 对常数平移不敏感。" >}}
...
## Stage 8 · Multi-Head Attention：前向拼列，反向切片再求和
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F12.svg" label="F12" caption="MHA 的反向规则：输出投影先回到 concat，再按列切给各 head，最后各 head 的输入梯度相加。" >}}
...
再把 $\Delta_{concat}$ 按列切回每个 head。每个 head 独立套 Stage 6 的反传，
...
## Stage 9 · LayerNorm：反向是在每个 token 行内做投影
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F13.svg" label="F13" caption="LayerNorm 的统计量沿 feature 维计算；反向也在每个 token 行内完成。" >}}
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F14.svg" label="F14" caption="LoRA 是冻结主干旁边的低秩可训练旁路；反向只更新 $D$ 与 $U$。" >}}
...
## Stage 11 · GPT Block：残差让梯度分流，也让推导可组合
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F15.svg" label="F15" caption="GPT block 的反向组合规则：每个 residual add 都把上游梯度复制到主支和残差支。" >}}
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F16.svg" label="F16" caption="推导查漏表：重写后保留的核心对象、反向入口和易错点。" >}}

| 推导点 | 资料位置 | 前向对象 | 反向关键 | 易错点 |
|---|---|---|---|---|
| SGD 与全梯度 | `BP.pdf` p.2-3 / CMU Lecture 12 | $L(\theta)=\sum_n C_n(\theta)$ | $\nabla_\theta L$ | 训练要整条梯度，不是单个偏导 |
| BP 两遍算法 | `BP.pdf` p.4-11, p.19 | $z=\sum_i a_iw_i+b$ | $\partial L/\partial w_i=a_i\Delta_z$ | forward 保存局部斜率，backward 递推 $\Delta_z$ |
...
2. 按 BP.pdf 的两遍算法读每条边：forward 给局部斜率，backward 给 $\Delta_z$。
3. 把模型写成计算图，标出哪些 forward 值需要在 backward 复用。
4. 对每个节点写出局部规则，并把多条下游路径的贡献相加。
5. 对矩阵层写微分 $dA$，用 $\Delta_A:dA$ 收集每个输入和参数的系数。
6. 对 softmax、LayerNorm、mask、residual 这类“结构性操作”单独检查约束。
