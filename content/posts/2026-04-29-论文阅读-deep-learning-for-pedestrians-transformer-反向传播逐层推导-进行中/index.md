---
title: "论文精读：Transformer 反向传播逐层推导，从 Δ·dA 到 Attention、LayerNorm 与 LoRA"
date: 2026-04-29T10:00:00+08:00
lastmod: 2026-05-02T23:10:00+08:00
draft: false
summary: "重写 Laurent Boué《Deep learning for pedestrians: backpropagation in Transformers》：不再逐段翻译，而是按符号、前向图、反向模板、Embedding、Self-Attention、MHA、LayerNorm、LoRA 与 GPT block 逐层重建 Transformer 反向传播。"
...
> 这版把原来的逐段英中翻译改成“推导路线图”。目标不是复刻论文全文，
> 而是把 Laurent Boué 这篇 37 页教程里最值得记住的反向传播结构整理成
> 一条可以手算、可以检查代码、也可以定位梯度 bug 的路径。
...
{{< tip >}}
**阅读方式**：先把每层写成前向对象 $A=f(X,\theta)$，再写
$dA$，最后把 $\Delta_A:dA$ 重新排列成 $\Delta_X:dX$ 和
$\partial L/\partial\theta:d\theta$。这就是全文的统一模板。
{{< /tip >}}

## Prologue · 这篇论文真正教的不是 Transformer，而是反向传播的读法
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F1.svg" label="F1" caption="论文重写后的阅读路线：从统一反传模板开始，再逐层推导 Transformer 组件。" >}}

本文采用四个约定：

| 符号 | 含义 |
|---|---|
| $X\in\mathbb R^{n_T\times d}$ | 一条序列的 token 表示，行是 token，列是特征 |
| $\Delta_A$ | 损失对中间量 $A$ 的 adjoint，形状与 $A$ 相同 |
...
## Stage 1 · 统一模板：每层 backward 都是整理 $\Delta:dA$

论文沿用前作 CNN backprop 的写法：不用厚重下标，而是用矩阵微分和形状来约束推导。
对任意一层
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F2.svg" label="F2" caption="反向传播的统一模板：先写微分，再把 $\Delta_A:dA$ 改写成各个变量的梯度。" >}}
...
## Stage 2 · Embedding：前向是查表，反向是 scatter-add
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F3.svg" label="F3" caption="Embedding 的 OHE 写法只是推导工具；实现里对应被选中行的 scatter-add。" >}}
...
## Stage 3 · Self-Attention 前向：先看对象，再看依赖
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F4.svg" label="F4" caption="单头 attention 的前向图：Q/K 决定注意力分布，V 被分布加权混合。" >}}
...
## Stage 4 · Self-Attention 反向：$\Delta_Y$ 分三条路回到 $X$
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F5.svg" label="F5" caption="Attention 反向：输出梯度先分到 V 与 P，P 再经 softmax 回到 Q/K score。" >}}
...
## Stage 5 · Key bias 为什么没有意义：softmax 的平移不变性
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F6.svg" label="F6" caption="Key bias 失效的原因：它只给每一行 score 加常数，而 row-softmax 对常数平移不敏感。" >}}
...
## Stage 6 · Multi-Head Attention：前向拼列，反向切片再求和
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F7.svg" label="F7" caption="MHA 的反向规则：输出投影先回到 concat，再按列切给各 head，最后各 head 的输入梯度相加。" >}}
...
再把 $\Delta_{concat}$ 按列切回每个 head。每个 head 独立套 Stage 4 的反传，
...
## Stage 7 · LayerNorm：反向是在每个 token 行内做投影
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F8.svg" label="F8" caption="LayerNorm 的统计量沿 feature 维计算；反向也在每个 token 行内完成。" >}}
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F9.svg" label="F9" caption="LoRA 是冻结主干旁边的低秩可训练旁路；反向只更新 $D$ 与 $U$。" >}}
...
## Stage 9 · GPT Block：残差让梯度分流，也让推导可组合
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F10.svg" label="F10" caption="GPT block 的反向组合规则：每个 residual add 都把上游梯度复制到主支和残差支。" >}}
...
{{< fig src="/figures/2026-04-29-论文阅读-deep-learning-for-pedestrians-transformer-反向传播逐层推导-进行中/F11.svg" label="F11" caption="推导查漏表：重写后保留的核心对象、反向入口和易错点。" >}}

| 推导点 | 论文覆盖位置 | 前向对象 | 反向关键 | 易错点 |
|---|---|---|---|---|
...
1. 先说清楚前向对象的形状和语义。
2. 写出微分 $dA$，不要跳步。
3. 用 $\Delta_A:dA$ 收集每个输入和参数的系数。
4. 对 softmax、LayerNorm、mask、residual 这类“结构性操作”单独检查约束。
