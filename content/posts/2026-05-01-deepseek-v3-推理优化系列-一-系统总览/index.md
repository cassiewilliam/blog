...
draft: false
summary: "从 DeepSeek Day 6 的 545% cost-profit margin 和 73.7K/14.8K tok/s 标尺出发，重建 V3/R1 推理系统的 PD 分离、Cross-node EP、DP/EP 边界、精度栈和负载均衡地图。"
categories: ["LLM 推理系统", "CUDA Hopper & Blackwell"]
tags: ["deepseek-v3", "llm-inference", "moe", "mla", "deepep", "deepgemm", "flashmla", "vllm", "deep-dive"]
math: true
drawio: true
ShowToc: true
TocOpen: true
...
{{< fig src="/figures/2026-05-01-deepseek-v3-推理优化系列-一-系统总览/F1.svg" label="F1" caption="Metric ledger: official numbers become engineering constraints" >}}
...
## Stage 1 · 系统层：PD 分离和 Cross-node EP
...
{{< fig src="/figures/2026-05-01-deepseek-v3-推理优化系列-一-系统总览/F2.svg" label="F2" caption="PD separation: throughput and latency get different EP sizes" >}}
...
{{< fig src="/figures/2026-05-01-deepseek-v3-推理优化系列-一-系统总览/F3.svg" label="F3" caption="DP / EP boundary: dense paths replicate; routed experts shard" >}}
...
{{< fig src="/figures/2026-05-01-deepseek-v3-推理优化系列-一-系统总览/F4.svg" label="F4" caption="Precision stack: FP8 where moving bytes, BF16 where reducing" >}}

## Stage 4 · 算子层：DeepGEMM 与 FlashMLA
...
{{< fig src="/figures/2026-05-01-deepseek-v3-推理优化系列-一-系统总览/F5.svg" label="F5" caption="System map: DeepEP, DeepGEMM, FlashMLA, DBO line up" >}}
...
{{< fig src="/figures/2026-05-01-deepseek-v3-推理优化系列-一-系统总览/F6.svg" label="F6" caption="vLLM reference: open-source path vs production service" >}}

## Stage 5 · 调度层：DBO 把单点优化串起来
...
{{< fig src="/figures/2026-05-01-deepseek-v3-推理优化系列-一-系统总览/F7.svg" label="F7" caption="Three balancers: prompt, KV, and expert hotspots are separate" >}}
...
|---|---|---|---|
| PD 分离 | prefill 与 decode 目标冲突 | 两套 deployment unit 与 EP size | 跨阶段 KV/cache 调度变复杂 |
| Cross-node EP | routed expert 权重与 token 峰值过大 | routed expert 切 EP，attention/shared 走 DP | RDMA latency 需要 DeepEP/DBO 隐藏 |
| 精度分层 | 通信带宽与 reduce 精度冲突 | FP8 dispatch，BF16 combine/MLA | 质量边界取决于量化 recipe |
| 三层 LB | 平均 batch 掩盖长尾 | prefill/decode/EPLB 分别均衡 | 在线实现需要持续观测和迁移 |


## Figure Set Review · 本篇图集一致性

{{< fig src="/figures/2026-05-01-deepseek-v3-推理优化系列-一-系统总览/F8.svg" label="F8" caption="Audit map: one series, eight lenses" >}}
