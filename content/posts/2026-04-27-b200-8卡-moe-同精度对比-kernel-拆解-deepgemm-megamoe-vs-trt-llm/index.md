...
{{< tip >}}
📊 **图表占位**：原文这里有 Chart.js 动态柱图（id=`chartSamePrecDSV4`）。
markdown 化时无法保留 JS 渲染；请用 drawio 重画一张静态版本，或参考原 commit 历史的 chart 数据。
{{< /tip >}}

{{< tip >}}
📊 **图表占位**：原文这里有 Chart.js 动态柱图（id=`chartSpeedupDSV4`）。
markdown 化时无法保留 JS 渲染；请用 drawio 重画一张静态版本，或参考原 commit 历史的 chart 数据。
{{< /tip >}}
...
{{< tip >}}
📊 **图表占位**：原文这里有 Chart.js 动态柱图（id=`chartSamePrecQ35`）。
markdown 化时无法保留 JS 渲染；请用 drawio 重画一张静态版本，或参考原 commit 历史的 chart 数据。
{{< /tip >}}

{{< tip >}}
📊 **图表占位**：原文这里有 Chart.js 动态柱图（id=`chartSpeedupQ35`）。
markdown 化时无法保留 JS 渲染；请用 drawio 重画一张静态版本，或参考原 commit 历史的 chart 数据。
{{< /tip >}}
...
{{< tip >}}
📊 **图表占位**：原文这里有 Chart.js 动态柱图（id=`chartKernel`）。
markdown 化时无法保留 JS 渲染；请用 drawio 重画一张静态版本，或参考原 commit 历史的 chart 数据。
{{< /tip >}}
